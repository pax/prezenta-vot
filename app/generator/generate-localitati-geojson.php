...
foreach ($participare_obj as $siruta => $one_loc) {
  ...
  $onePoint->props->pe_lista  = 0;
   $onePoint->props->sectii = 0;
  foreach ($participare_obj->$siruta->sectii as $nr_sectie => $oSectie) {
    $onePoint->props->sectii++;
    ...
    $onePoint->props->pe_lista += $oSectie->{'pe lista'};
    foreach ($oSectie->ts as $ts => $tsVals) {
      ...
      foreach ($tsVals as $valName => $oneVal) {
        ...
        if (!isset($onePoint->props->total->$ts))  $onePoint->props->total->$ts = new \stdClass();
        if (!isset($onePoint->props->total->$ts->$valName)) $onePoint->props->total->$ts->$valName = 0;
        $onePoint->props->total->$ts->$valName +=  + $oneVal;
      }
    }
  }
