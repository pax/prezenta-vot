...
function pr ($array, $is_shy=0, $style='array'){
  ...
  if ($is_shy) $shy='shy';
	...
	switch ($style){
		case 'dump': var_dump($array);
		case 'export': var_export($array);
		default: print_r($array);
	}
	echo '</div></pre>';
}
...
function sanitizeText ($input) {
	...
	if ($encoding)	$output=$input;
		else if ($encoding=='ASCII') $output=htmlentities(mb_convert_encoding($input, 'UTF-8', 'auto'), ENT_SUBSTITUTE, "UTF-8");
		else if ($encoding=='UTF-8') $output=htmlentities($input, ENT_SUBSTITUTE, "UTF-8");
